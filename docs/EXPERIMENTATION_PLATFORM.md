...
Unknown or missing required fields are rejected where a safety decision depends on the payload. Supported older schemas are migrated explicitly; unsupported schemas remain unchanged and report a diagnostic. Fingerprints use canonical, secret-free content and do not depend on display names or capture timestamps. `ai_arena.qa_seal_manifest.v1` fixes the required global and per-pass gates, registered schema checks, nested-repository provenance, performance/resource metrics, and screenshot/automation artifact kinds so a producer cannot self-declare a smaller suite as sealed.
...
UI-structure v1 evidence now requires every privacy-safe node identity to be unique. Repeated template names and repeated redacted automation IDs receive a bounded visible-tree ordinal suffix. Older ignored local bundles that predate this invariant are deliberately rejected and must be regenerated; QA bundles are source-fingerprint-bound evidence, not an archival interchange format.
