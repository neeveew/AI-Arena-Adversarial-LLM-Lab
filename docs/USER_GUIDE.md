...
- **Scenario Packs** stores canonical, versioned scenario and benchmark artifacts separately from personal templates. Imports are size-bounded, schema-checked, migration-aware, and reject duplicate content identities.
...
- **Routing Optimizer** reads only compatible completed experiment and evaluation evidence. It first creates a proposal; applying that exact proposal requires explicit approval and a final setup/current-route concurrency check.
- **In-App QA Inspector** reads ignored local Quality Seal bundles, validates hashes and linked render/WPF-structure evidence, runs allowlisted focused suites, and produces privacy-safe reports. Full sealing remains a post-close command. Acceptance is unavailable until every referenced screenshot has been explicitly previewed; the review manifest is tied to the evidence-file, source-tree, and artifact hashes and is cleared by refresh or change. The compatibility matrix proves off-screen raster density, programmatic in-process WPF focus/structure, and motion-preference plumbing—not physical display DPI, OS keyboard/UI Automation interaction, or animation playback over time. Source fingerprints are checked at bounded build/pass/validator boundaries, but the run does not occur in an immutable worktree, so a transient edit-and-restore between checks is an explicitly accepted fourth limitation. Direct script acceptance requires `-AttestReviewedVisuals` and records a human attestation rather than claiming the script saw the pixels.
