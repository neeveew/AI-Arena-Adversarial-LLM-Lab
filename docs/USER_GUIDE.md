...
6. Press **Models** in the AI Lab top rail. Select a loaded or available model, then check **Default** to enable the shared fallback for unassigned Arena agents, or check an active agent/Narrator target for an explicit route. Assignment changes save immediately.
...
The left rail contains the app identity, navigation, and contextual information. AI Lab, Agent, and AI Collaborate are first-class navigation surfaces. Agent is visible by default and can be shown or hidden independently under **Settings -> Agent workspace**; this does not require Debug controls. In AI Lab the rail shows session overview and live agent status. In Agent it shows the active working directory and software team roles. In AI Collaborate it shows recent collaboration chats and team roles.
...
- Arena Controls.
- Universal Status Center, fixed immediately below Arena Controls.
- Agent Performance.
- Model Comparison & QA.
- Operator Turn.
...
- When the right rail is hidden, the compact status affordance opens the same Universal Status Center history without resizing the workspace.
...
Auto Chat stops when you press Pause or when a provider error occurs. A typed **Context limit reached** card means the provider rejected that causal prompt; AI Arena does not silently switch models, summarize it, or trim retained text. Use **Increase context**, **Choose larger model**, **Start fork**, **Skip turn**, or **End match**. A typed **Output limit reached** card preserves the partial response and offers **Continue** or **Increase output**. End Match durably disables Auto Chat, 1 Turn, Narrate, and individual agent actions until you reset or fork the session.

### Universal Status Center

The fixed card below Arena Controls shows four compact rows with the newest
activity first. It combines meaningful Arena, Agent, Collaborate, Models,
provider, save, export, and copy feedback without moving nearby controls.
Unresolved blockers remain visible, routine healthy heartbeats stay quiet, and
the idle state reads **Ready**.

Select a row to open the compact history dashboard to the left. Filter it by
All, Running, Warnings, or Failed, inspect details and progress, and use an
available destination action to return to the relevant surface. **Clear
completed** removes only clearable completed entries; running work and
unresolved warnings or failures remain. History is bounded to the current app
run and is not written into the session.
...
Open **Experiment Lab** from the left navigation rail. Its feature selector contains only workspaces backed by real local services; a missing source is shown as unavailable rather than filled with inferred data.

- **Matrix Runner** expands a versioned experiment into deterministic provider, dimension, and repetition cells. It resolves the active persisted session, exact scenario pack, rubric versions, provider profiles, and supported tuning axes before enabling Run. Runs use isolated child sessions, bounded concurrency, durable restart state, cancellation, and an all-or-nothing capacity check. The matrix definition is saved separately from its per-cell history. After an app/process restart, an abandoned matrix is restored as **Interrupted**; clicking Run without **Approve one retry of terminal cells** starts zero cells and leaves it unchanged. Approval is one-shot: an Interrupted or Cancelled continuation retains earlier trials, child-session references, evidence, and every already-Completed cell while completing missing or non-completed terminal cells. If every cell finished just before a crash, approval closes the definition as Completed with zero duplicate provider calls. Approving an intentionally Completed definition is a deliberate full repeat.
- **Conversation Fork** creates an isolated child at a stable transcript message. Transcript, narration, and provenance-bearing memory are projected conservatively; later, ambiguous, expired, or unprovable private memory is omitted. Existing session files do not contain historical provider, persona, steering, relationship, or cast checkpoints, so the child retains the current replayable setup and the receipt explicitly marks historical setup projection unavailable.
- **Scenario Packs** stores canonical, versioned scenario and benchmark artifacts separately from personal templates. Imports are size-bounded, schema-checked, and reject duplicate content identities. Import explicitly supports only the recognized scenario-pack v0 and benchmark-pack v0 shapes: the receipt shows the source schema/version, a short exact-source SHA-256, fixed migrator version, and UTC migration time. The importer recomputes and validates v1 identity; it never stores the selected path or legacy source bytes, and unsupported/hostile input remains rejected.
- **Rubric Studio** keeps human, deterministic, model-judge, and unavailable observations distinct. Rubric versions are immutable, derived scores are recomputed before persistence, and disagreement remains visible. **Run provider judge** evaluates an exact transcript message using that message speaker's effective configured provider route; **Cancel judge** remains available while that provider request is active. Cancellation before finalization stores no receipt or result. Once provider completion has yielded a finalization, the bounded local receipt-first write completes rather than leaving a result without its proof. Only strict criterion JSON tied to the matching succeeded request receipt becomes an inferred model score; typed model scores are not accepted. For a human blind comparison, enable **Blind pairwise comparison**, enter two exact transcript message IDs, and choose **Begin concealed comparison**. The commitment is saved before both message texts appear under opaque A/B labels; their IDs remain hidden until **Submit blind judgment** saves both the receipt and result. Cancelling records no result and leaves the commitment as an unclaimed audit artifact; restart the two-step flow if the app closes before submission.
- **Claim Ledger** links bounded claims to stable transcript messages and preserves monotonic source, contradiction, reviewer, and status-transition evidence.
- **Context & Prompt Inspector** shows process-memory provider attempts grouped by correlation ID: exact outbound-body hash and byte count, redacted preview, role transformations, context omissions, retry phase, and honest provider-reported, estimated, or unavailable token evidence. Clear affects traces only.
- **Agent Memory Debugger** reveals private memory only after one agent is selected. It shows origin, visibility, branch, revision, expiry, correction, and source provenance; additions, corrections, and expiry use the same structured-memory persistence path as arena turns.
- **Fault-Injection Lab** arms a bounded process-only decorator for deterministic timeout, disconnect, malformed-stream, saturation, empty-response, interruption, or context-pressure probes. Injected cause and observed recovery are reported separately.
- **Routing Optimizer** reads only compatible completed experiment and evaluation evidence from each run's latest trial under the current resolved plan. Older attempts and attempt-agnostic run judgments are not reused after a retry. Historical trials establish chat capability, not the current machine's hardware; without separately observed current-hardware evidence, the optimizer reports insufficient evidence instead of proposing a change. Applying an eligible exact proposal requires explicit approval and a final setup/current-route concurrency check.
- **In-App QA Inspector** reads ignored local Quality Seal bundles, validates hashes and linked render/WPF-structure evidence, runs allowlisted focused suites, and produces privacy-safe reports. Full sealing remains a post-close command. Acceptance is unavailable until every referenced screenshot has been explicitly previewed; this automatically includes the 10-feature × 3-theme × 2-width cells from every clean pass (120 screenshots for the required two passes). Each feature cell comes from a real local control-plane selection and completed refresh and must show exactly the selected feature's rendered content root, with non-zero bounds, opacity, viewport intersection, feature-local focus traversal, and materially distinct pixels from the other features at the same theme and width. Current seals bind those fields to `ui_structure_evidence.v2`; historical V1 seal bundles retain their original legacy structure shape and cannot be substituted into current evidence. The review manifest is tied to the evidence-file, source-tree, and artifact hashes and is cleared by an ordinary interactive refresh or evidence change. The isolated QA capture process opens the Inspector in read-only mode: suite and acceptance commands are disabled and any existing valid completed review manifest is preserved byte-for-byte. The separate compatibility matrix proves off-screen raster density, programmatic in-process WPF focus/structure, and motion-preference plumbing—not physical display DPI, OS keyboard/UI Automation interaction, or animation playback over time. Source fingerprints are checked at bounded build/pass/validator boundaries, but the run does not occur in an immutable worktree, so a transient edit-and-restore between checks is an explicitly accepted fourth limitation. Direct script acceptance requires `-AttestReviewedVisuals` and records a human attestation rather than claiming the script saw the pixels. Historical V1 seal manifests remain valid historical evidence; current sealing and acceptance require V2's executable migration authority and feature-surface matrix.

Experiment Lab never sends runtime data to a cloud service on its own. Provider calls occur only for an explicitly started matrix, provider-backed judge action, or fault probe, using the active provider configuration.
...
Choose a workspace folder from the collapsed Workspace drawer. The active path is shown in the page header and left rail, and command previews use that folder as the working directory. The selected path is saved in WPF settings. Agent builds a capped workspace profile from common project files such as `package.json`, `.sln`, `.slnx`, `.csproj`, `pyproject.toml`, `Cargo.toml`, `go.mod`, and `index.html`; Planner, Reviewer, and Builder receive that profile so they can choose better first commands and verification steps without scanning large dependency folders. For .NET workspaces, the native Solution Doctor also identifies solution membership, target frameworks, project references, output types, WPF projects, test-SDK projects, executable test harnesses, and missing restore assets. It emits deterministic root-cause findings for proven project-reference cycles and clearly incompatible target frameworks, plus medium-confidence direct package-version conflicts only when differing canonical literal versions coexist in one directed transitive project-reference closure. Static framework compatibility is deliberately bounded to fully parsed modern `net5.0+`, `netcoreapp`, and `netstandard` targets: same-family consumers must be at least the dependency version; modern .NET supports `netstandard` through 2.1; and `netcoreapp` 1.x, 2.x, and 3.x or later support `netstandard` through 1.6, 2.0, and 2.1 respectively. Fully parsed platform qualifiers and minimum versions are also compared. Conditional, imported, central, legacy .NET Framework, custom, partially evaluated, or otherwise unknown cases remain unclassified instead of being guessed. All finding and evidence paths are workspace-relative, and partial discovery is reported explicitly.

Open **Impact Explorer** in the Agent right rail for opt-in Roslyn analysis of a trusted .NET solution. Analysis never starts merely because a workspace is selected: press **Analyze** after entering a type, member, test, or workspace-relative source file, or press **Last Changes** to use the latest bounded Agent file receipt. Loading a solution evaluates its MSBuild project files, so use this only with code you trust. The result shows matched definitions and relative locations; callers, callees, incoming/outgoing references, base types, interfaces, and derived implementations; project/package, XAML, and test relationships; and bounded predictions for affected files and feature areas. Partial restores, unsupported projects, traversal limits, and missing history remain labelled partial or unavailable.

Impact Explorer’s focused tests are static recommendations, not runtime coverage. **Stage Tests** places the first structurally validated project/test command into the existing Command Approval rail; it does not execute it. Review the exact command and approve it explicitly. “No static test relationship” means only that the bounded semantic graph supplied no relationship—it does not prove a production change is untested. **Copy Impact** exports the same relative-path-only evidence and limitation labels.

For supported Solution Doctor findings, **Stage Repair** first produces an explanation and a bounded proposed diff. The diff is review evidence, not permission to write: repair commands always require an explicit click even when Full Access is enabled, and a stale file hash invalidates the proposal. After an approved repair, **Stage Verify** offers focused `--no-restore` project builds and only dependency-evidenced test harnesses. Verification also requires explicit approval. The result compares the finding set before and after and labels findings fixed, unchanged, regressed, new, partial, or cancelled. Only a completed, explicitly approved repair-and-verification loop writes bounded metadata to the ignored local `.ai-arena/analysis/solution-doctor-history.json`; it never stores source, raw commands, diffs, absolute paths, settings, or secrets. Static test selection and successful execution are evidence, not a runtime-coverage claim.
...
- Build Evidence shows whether the workspace is ready, a command is required, a proposal exists, preview is ready, a command ran, files changed, a likely artifact was detected, and the next action is verify or repair. In .NET workspaces it adds project/discovery state, bounded Solution Doctor finding counts, structured C#/MSBuild/NuGet/.NET SDK warning and error totals, test totals, explicit test-discovery failure, and the first safe relative failure location.
...
Only supplied provider fields change; explicit `$false`, numeric `0`, and empty role-model values are preserved. An empty role model clears that explicit route; it uses the shared model only while `DefaultForUnassignedAgentsEnabled` is true, and otherwise remains unassigned. A nonblank role model is explicit even when it equals the shared model. To save a credential without placing plaintext in command history, use `$providerToken = Read-Host "Provider token" -AsSecureString`, then `Set-AIArenaProviderConfig -ApiToken $providerToken`. Provider responses and events expose only whether a token is configured. `Set-AIArenaProviderModel` remains the shortcut for assigning one model to the shared route and every arena role.
...
Current-match forking is additive and needs no confirmation, but it is refused while the arena is busy. It branches the authoritative current snapshot. For an older transcript cursor, use **Experiment Lab → Conversation Fork**: it truncates transcript and narration and projects structured memory conservatively, while explicitly retaining the current setup because cursor-scoped setup history is unavailable. Fork receipts contain lineage IDs, revisions, omission counts, evidence states, and time only; provider credentials, transcript text, and memory text are not returned.
...
Copy Setup copies the current match setup as a readable brief with readiness, topic, global rules, recipe, preset match, run shape, relationship map, locks, cast, narrator, and provider context. Copy JSON emits the exact setup as a portable `ai_arena.match_setup.v4` package. V4 records whether unassigned Arena agents may use the shared model, marks every provider role as `explicit` or `inherit`, and carries canonical per-model context/history/tone settings—including configured catalog models that are not currently assigned, using bounded safe model IDs. Import re-keys those settings against the destination connection. Import JSON also accepts v2/v3 with strict/default legacy behavior before validation. It validates every field before writing anything, then creates and selects a clean session. It never replaces the current run. Packages carry scenario, generation tuning, cast, narrator, locks, relationship pressure, context, Internet policy, and non-secret provider settings; API tokens, absolute local paths, runtime transcript/history, and transient residency evidence are excluded.
...
Each model has one durable behavior configuration across all agents that route to it: Provider default or a 512-1,048,576 context window, strict or Rolling 80% Arena history, and a response tone. Rolling 80 keeps the newest whole eligible Arena-history entries that fit an estimated 80% input budget while reserving space for output; it never trims an included entry or creates a hidden summary. Each attempted turn records privacy-safe estimated-token, included/omitted-entry, boundary, and fingerprint evidence. The provider remains authoritative if its tokenizer disagrees. Rolling 80 is saved but inactive when Context window is **Provider default**, because the app has no truthful numeric budget; set an explicit window to activate it. Factory public group history remains governed by `public_group_v1` and ignores the Arena history policy. Saving these fields never assigns, loads, or unloads the model. If a loaded LM Studio process uses a different effective context—or an explicit override is changed back to Provider default—**Reload to apply** performs an explicit unload/load. Definite rejections are shown as failed, ambiguous post-request outcomes as unconfirmed, and success only after provider residency evidence confirms the replacement instance; a Provider-default reload does not invent a numeric active-context claim.
...
Checking **Default** saves the selected model as the shared route and enables it as the fallback for unassigned Arena agents. Unchecking **Default** keeps the shared model configured but disables that fallback, so every Arena participant that should run needs an explicit assignment. Checking an active Alpha-Theta participant or Narrator creates an explicit assignment, including when the selected model equals Default; unchecking it returns the role to inherited-or-unassigned behavior according to the Default toggle. There is no Apply or Reset button, and assigning a model does not load or unload it.
...
- **Saved setups**: save or reuse provider and role-routing configurations, including explicit routes for active Alpha-Theta roles and the Default fallback toggle. New and legacy profiles default that toggle on, and legacy Alpha-Delta/Narrator fields migrate into the role map; access tokens are never included.
