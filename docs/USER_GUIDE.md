...
- Build Evidence shows whether the workspace is ready, a command is required, a proposal exists, preview is ready, a command ran, files changed, a likely artifact was detected, and the next action is verify or repair. In .NET workspaces it adds project/discovery state, structured C#/MSBuild warning and error totals, test totals, and the first safe relative failure location.
