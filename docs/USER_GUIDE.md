...
Copy Setup copies the current match setup as a readable brief with readiness, topic, global rules, recipe, preset match, run shape, relationship map, locks, cast, narrator, and provider context. Copy JSON emits the exact setup as a portable `ai_arena.match_setup.v3` package. V3 records whether unassigned Arena agents may use the shared model and marks every provider role as `explicit` or `inherit`; an explicit role can intentionally equal the shared model. Import JSON also accepts v2, enabling the shared fallback and inferring legacy role modes before validation. It validates every field before writing anything, then creates and selects a clean session. It never replaces the current run. Packages carry scenario, generation tuning, cast, narrator, locks, relationship pressure, context, Internet policy, and non-secret provider settings; API tokens and runtime transcript/history are excluded.
