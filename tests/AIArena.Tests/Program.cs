...
var tests = new List<(string Name, Action Test)>
{
    ...
    ("experiment contracts share canonical UTF-8 validation hashing and storage bytes", ExperimentPersistenceOptimizationTests.CanonicalUtf8FacadesPreserveBytesPrivacyAndAllocations),
    ("experiment run loading is bounded deterministic and cancellation safe", ExperimentPersistenceOptimizationTests.RunLoadingIsBoundedDeterministicAndCancellationSafe),
    ("experiment run loading preserves restart normalization and resource bounds", ExperimentPersistenceOptimizationTests.RunLoadingPreservesRestartNormalizationAndResourceBounds),
    ...
    ("solution doctor diagnoses dependency root causes deterministically", DotNetSolutionDoctorTests.DiagnosesDependencyRootCausesDeterministically),
    ("solution doctor diagnoses package downgrade and test discovery failure", DotNetSolutionDoctorTests.DiagnosesPackageDowngradeAndTestDiscoveryFailure),
    ("solution doctor scopes dependency findings to directed complete projects", DotNetSolutionDoctorTests.ScopesDependencyFindingsToDirectedCompleteProjects),
    ("solution doctor normalizes framework and package evidence conservatively", DotNetSolutionDoctorTests.NormalizesFrameworkAndPackageEvidenceConservatively),
    ("solution doctor requires exact runtime failure evidence", DotNetSolutionDoctorTests.RequiresExactRuntimeFailureEvidence),
    ...
    ("rolling context records scalable selection performance", ContextWindowTests.RollingBudgetPerformanceReceipt),
    ("rolling context optimized selector matches the legacy oracle", ContextWindowTests.RollingBudgetOptimizedMatchesLegacyOracle),
    ("rolling context production contracts preserve exact prompt bytes", ContextWindowTests.RollingBudgetProductionContractsPreservePromptBytes),
    ("diagnostics ordered and bounded selection matches legacy oracle", DiscourseDiagnosticsOptimizationTests.OrderedAndBoundedSelectionMatchesLegacyOracle),
    ("diagnostics records bounded selection performance", DiscourseDiagnosticsOptimizationTests.PerformanceReceiptShowsBoundedSelection),
    ("rolling context contradicted assertion falls back exactly", ContextWindowTests.RollingBudgetContradictedAssertionFallsBackExactly),
    ("prepared turn preserves exact bytes and freezes causal inputs", PreparedTurnContextTests.PreservesExactPromptBytesAndFreezesCausalInputs),
    ("prepared turn records one pass and a fixed expiry clock", PreparedTurnContextTests.RecordsSinglePreparationPassAndFixedExpiryClock),
    ...
    ("prompt inspector records metadata-first performance receipt", PromptInspectorTests.MetadataFirstTracingPerformanceReceipt),
    ("prompt inspector captures metadata until an inspector subscribes", PromptInspectorTests.CapturesMetadataUntilAnInspectorSubscribes),
    ("prompt inspector redacts provider trace privacy fuzz", PromptInspectorTests.RedactsProviderTracePrivacyFuzz),
    ("prompt inspector preview subscriptions are thread safe and bounded", PromptInspectorTests.PreviewSubscriptionsAreThreadSafeAndTraceStoreIsBounded),
    ...
    ("search DNS validation deduplicates hosts with bounded receipt", InternetSecurityTests.SearchDnsValidationDeduplicatesHostsWithBoundedReceipt),
    ("search DNS validation is bounded cancelable and fetches revalidate", InternetSecurityTests.SearchDnsValidationIsBoundedCancelableAndFetchesRevalidate),
    ...
    ("bounds unique snapshot mutation stamps across eviction and concurrency", SessionStorePersistenceOptimizationTests.BoundsUniqueSnapshotMutationStampsAcrossEvictionAndConcurrency),
    ("streams persistence revisions with legacy semantics and bounded receipts", SessionStorePersistenceOptimizationTests.StreamsRevisionWithLegacySemanticsAndBoundedReceipts),
    ("preserves corrupt and duplicate revision semantics", SessionStorePersistenceOptimizationTests.PreservesCorruptAndDuplicateRevisionSemantics),
    ("serializes protected tokens without cloning provider configs", SessionStorePersistenceOptimizationTests.SerializesProtectedTokensWithoutCloningConfigData),
    ("keeps snapshot checkpoint and temporary files private", SessionStorePersistenceOptimizationTests.KeepsSnapshotCheckpointAndTemporaryFilesPrivate),
    ("protector failures roll back without publishing plaintext", SessionStorePersistenceOptimizationTests.ProtectorFailuresRollbackWithoutPublishingPlaintext),
    ...
    ("event log ordered batches preserve legacy bytes and record performance", EventLogBatchingOptimizationTests.OrderedBatchMatchesLegacyBytesAndRecordsPerformance),
    ("event log batching preserves cancellation and crash prefixes", EventLogBatchingOptimizationTests.CancellationAndCrashPrefixRemainSafe),
    ("event log batching preserves rotation concurrency and session isolation", EventLogBatchingOptimizationTests.RotationConcurrencyAndSessionsRemainOrderedAndIsolated),
