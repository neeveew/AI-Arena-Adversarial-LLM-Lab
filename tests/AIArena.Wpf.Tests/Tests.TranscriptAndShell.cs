...
static void ReleaseScriptsProtectInstallerDistributions()
{
    ...
    Require(releaseSecurityScript.Contains("Find-AIArenaCodeSigningCertificates", StringComparison.Ordinal) && releaseSecurityScript.Contains("selection remains explicit", StringComparison.OrdinalIgnoreCase), "certificate discovery must not silently select a local development certificate");
    Require(releaseSecurityScript.Contains("Assert-AIArenaCertificatePrivateKey", StringComparison.Ordinal) && releaseSecurityScript.Contains("Assert-AIArenaCertificateChain", StringComparison.Ordinal), "signing preflight should verify the selected certificate chain and exercise its private key");
    Require(releaseSecurityScript.Contains("'verify', '/pa', '/all', '/v', '/tw'", StringComparison.Ordinal), "every signed artifact should be verified under the Windows Authenticode policy with a timestamp warning");
    Require(releaseSecurityScript.Contains("not the requested certificate", StringComparison.Ordinal) && releaseSecurityScript.Contains("required RFC 3161 timestamp", StringComparison.Ordinal), "post-sign verification should require the selected signer and RFC 3161 timestamp");
    Require(releaseScript.Contains("certificateChainTrusted", StringComparison.Ordinal) && releaseScript.Contains("privateKeyVerified", StringComparison.Ordinal) && releaseScript.Contains("timestampVerified", StringComparison.Ordinal), "release signing reports should attest certificate and timestamp preflight");
    ...
    Require(sanityScript.Contains("Assert-AIArenaSigningPolicyState", StringComparison.Ordinal) && releaseSecurityScript.Contains("Signing policy 'Required' cannot be recorded with signing disabled.", StringComparison.Ordinal) && releaseSecurityScript.Contains("Signing policy 'Disabled' cannot be recorded with signing enabled.", StringComparison.Ordinal), "release sanity should enforce the Required, Disabled, and Optional signing-state contract");
