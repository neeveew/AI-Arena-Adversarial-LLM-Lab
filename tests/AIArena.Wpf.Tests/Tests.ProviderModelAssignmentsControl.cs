...
internal static partial class Program
{
    static void ProviderModelAssignmentsControlHostsResponsiveAccessibleContract()
    {
        RunStaTest(() =>
        {
            ...
            try
            {
                ...
                Require(masterShare is >= 0.70 and <= 0.74,
                    $"wide Models surface did not preserve the 72/28 hierarchy (master share {masterShare:P1})");
                ...
                Require(groups.SequenceEqual(["Loaded Models", "Available Models", "Load State Unavailable"]),
                    "provider models were not grouped Loaded, Available, then Load State Unavailable");
                ...
                Require(AutomationProperties.GetName(control) == "Models and assignments"
                        && !control.Focusable
                        && AutomationProperties.GetName(control.CatalogList) == "Provider models by load state"
                ...
                FlushProviderModelsDispatcher(host);
                control.CatalogList.SelectedIndex = 0;
                FlushProviderModelsDispatcher(host);
                ...
                Require(control.FocusCatalog() && control.CatalogList.IsKeyboardFocusWithin,
                    "Models catalog was not keyboard focusable");
            }
            finally
            {
                host.Close();
            ...
            Require(ProviderModelAssignmentsControl.UsesCompactLayoutAt(960, 960)
                ...
                "Models responsive policy did not distinguish shell viewport from center-content width");
        });
    }
    ...
    static void ProviderModelAssignmentsControlRaisesImmediateTruthfulEvents()
    {
        RunStaTest(() =>
        {
            ...
            try
            {
                ...
                Require(alphaAssignment.ModelId == "loaded-000"
                ...
                Require(!control.RefreshAction.IsEnabled
                        ...
                        && control.CatalogList.Items.Count == 2
    ...
    static void ProviderModelAssignmentsControlHostsCausalLmStudioLifecycleAction()
    {
        RunStaTest(() =>
        {
            ...
            try
            {
                ...
                control.CatalogList.SelectedIndex = 0;
                control.CatalogList.ScrollIntoView(control.CatalogList.SelectedItem);
                FlushProviderModelsDispatcher(host);
    ...
    static ProviderModelAssignmentsPresentation ProviderModelsPresentation(int modelCount)
    {
        ...
        return new ProviderModelAssignmentsPresentation(
            ...
            models,
            [
                new ProviderAssignmentTargetPresentation("default", "Default", "Used by targets without an explicit route."),
